...
--
-- Run as the Postgres SUPERUSER inside the existing `postgres-db-1` container:
--     docker exec -i postgres-db-1 psql -U postgres < init-db.sql
--
-- SAFETY: this only CREATEs a brand-new role `eve_spai` and a brand-new database
-- `eve_spai`. It MUST NOT touch the co-tenant databases `postgres`, `nexus`, or
-- `nexus_users` (the entropia-nexus project) — there are no DROP/ALTER/GRANT against
-- them here. The schema (tables, indexes) is applied automatically by the server on
-- startup via embedded sqlx migrations, so nothing schema-related belongs in here.
--
-- BEFORE RUNNING — confirm the names are free (should print no rows):
--     docker exec -i postgres-db-1 psql -U postgres -c "\du eve_spai"
--     docker exec -i postgres-db-1 psql -U postgres -c "\l eve_spai"
-- AND confirm the neighbours exist and are untouched afterwards:
--     docker exec -i postgres-db-1 psql -U postgres -c "\l" | grep -E 'nexus|postgres'
...
-- the neighbouring databases are deliberately left alone.
